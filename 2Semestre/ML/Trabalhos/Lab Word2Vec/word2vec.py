...
def read_and_train():
	...
	model = gensim.models.Word2Vec (documents, size=VectorSize, window=WindowSize, min_count=MinCount, workers=10)
...
def extract_features_mean(model, reviews):
	result = []
...
if __name__ == "__main__":
	...
	reviews = read_csv()
	if m==1:
		results = extract_features_mean(model, reviews)
	elif m==2:
		results = extract_features_median(model, reviews)
	else:
		print ("1 para media e 2 para mediana")
		exit()

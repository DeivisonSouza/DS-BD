...
```{r,eval=TRUE, dpi= 600}
g1<-ggplot(Canal.1, aes(x=dias, y=fcuminsc)) + geom_step(linetype=1, lwd=0.8)+
...
ggsave("g1.jpeg", dpi = 900, width = 10, height = 5, units = "in")
```

# Visualização gráfica: Curva Real versus Curva Ajustada (Canal-2)

```{r,eval=TRUE, dpi= 600}
ggplot(Canal.2, aes(x=dias, y=fcuminsc)) + geom_step(linetype=1, lwd=0.8)+
...
O método de otimização de Nelder e Mead (1965) foi usado através da função `optim()`. Foram otimizados dois modelo: linear e logístico. As bondades de ajustes foi avaliada atráves da função perda quadrática (SQ). Para ambos os modelos a função logística foi a que melhor descreveu o número de incritos nos canais do YouTube ao longo do tempo. Isso foi ratificado graficamente e também pela avaliação do valor da perda quadrática.
...
`SQ` = `r round(fit_ols_C1$value[1],3)` (**Modelo Linear**)

`Parâmetros` = `r round(fit_ols_C1$par,3)` (**Modelo Linear**)


`SQ` = `r round(fit_logit_C1$value[1],3)` (**Modelo Logístico**)

`Parâmetros` = `r round(fit_logit_C1$par,3)` (**Modelo Logístico**)

- **Canal-2**:

`SQ` = `r round(fit_ols_C2$value[1],3)` (**Modelo Linear**)

`Parâmetros` = `r round(fit_ols_C2$par,3)` (**Modelo Linear**)

`SQ` = `r round(fit_logit_C2$value[1],3)` (**Modelo Logístico**)

`Parâmetros` = `r round(fit_logit_C2$par,3)` (**Modelo Logístico**)
